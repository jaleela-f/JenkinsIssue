	...
	static ConjurSecretCredentials credentialWithID(String credentialID, ModelObject context) {
		...
		if (credential == null) {
			...
			for (Item item : itemList) {
				LOGGER.log(Level.FINE, "All items in the list" + item);

				if (item.equals(((Run<?, ?>) context).getParent())) {
			...
			if (toCheck != null) {
				LOGGER.log(Level.FINE, "Get all jobs" + toCheck.getAllJobs());
				// String allJobs = toCheck.getAllJobs().toString();

				Collection<Job> listJob = new ArrayList(toCheck.getAllJobs());
				for (Job job : listJob) {

					LOGGER.log(Level.FINE, "Get all Jobs in folder>>>" + job);
				}

				String[] spiltJob = toCheck.toString().split("/");
				...
				for (int i = 0; i < spiltJob.length; i++) {
					...
					{
						LOGGER.log(Level.FINE, "Exception logged continue with next folder"+ex);
					}
				}
